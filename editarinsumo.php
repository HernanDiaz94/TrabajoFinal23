...
$error = false;
    ...
    if ($_GET['id']) {
        $conexion1 = new Mongo();
        $resultado = $conexion1->find($_GET['id'], 'insumos');
        
            $nombre_insumo= $resultado[0]->nombre_insumo;
            $codigo = $resultado[0]->codigo;
            $tipo = $resultado[0]->tipo_insumo;
    ...
    if (!$error) {
        ?>
            ...
            <form action="grabar_insumo.php" method="POST" enctype="multipart/form-data">
                
            <label class='control-label' for="id">ID</label><br>
            <input class='form-control' type="text" name="id" id="id" value="<?=$_GET['id']?>"><br><br>
            
            <label class='control-label' for="">Nombre del Insumo</label><br>
      <input required  class='form-control' name="nombre_insumo" type="text" value="<?=$nombre_insumo?>">
      <label class='control-label' for="">Codigo</label><br>
      <input required  class='form-control' name="codigo" type="text" value="<?=$codigo?>">
      <label class='control-label' for="">Tipo</label><br>
      <input required  class='form-control' name="tipo_insumo" type="text" value="<?=$tipo?>">
      <label class='control-label' for="precio">Descripcion</label><br>
                ...
                <select required  class='form-control' name="oficina" id="">
                  <?php
                      $resultados = $conexion->select([], 'oficinas');
                      if ($resultados) {
                        foreach($resultados as $r) {
                    ?>
                      <option value="<?=$r->codigo?>" <?=$oficina == $r->codigo ? 'selected' : '' ?>><?=$r->nombre_oficina?></option>  
                    <?php
                        }
                      }?> 
